...
class ZhihuSearchSpider(scrapy.Spider):
    ...
    keyword = '全栈'
    # offset 每20就翻一页
    start_urls = ['https://www.zhihu.com/api/v4/search_v3?gk_version=gz-gaokao&t=general&q=%E5%85%A8%E6%A0%88&correction=1&offset=0&limit=20&filter_fields=&lc_idx=0&show_all_topics=0&search_source=Normal']
    cookies = {
        ...
        "Hm_lpvt_98beee57fd2ef70ccdd5ca52b9740c49": "1724403472"
    }
    headers = {
        "accept": "*/*",
    ...
    def parse(self, response):
        ...
        with open('zhihu_spider/output.json', 'w', encoding='utf-8') as f:
            f.write(str(data))

        # 检查是否包含 `data` 字段
        if 'data' in data:
            for item in data['data']:
                # 检查对象是否包含 `question` 字段
                if 'object' in item and 'type' in item and item['type'] == 'search_result':
                    ...
                    if question:
                        ...
                        self.logger.warning("No 'question' field in the response(第二层).")
                else:
                    self.logger.warning("No 'question' field in the response.")
        else:
            self.logger.warning("No 'data' field in the response.")
